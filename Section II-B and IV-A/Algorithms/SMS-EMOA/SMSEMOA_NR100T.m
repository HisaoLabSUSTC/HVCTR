function SMSEMOA_NR100T(Global)
% <algorithm> <S>
...
%--------------------------------------------------------------------------
    ...
    while Global.NotTermination(Population)
        for i = 1 : Global.N
            ...
            Population = Reduce_New([Population,Offspring],r);
        end
    end
end

function Population = Reduce_New(Population,ref)
% Delete one solution from the population
    ...
    deltaS = CalHVC_New(PopObj,ref,N);
...
function HVC = CalHVC_New(data,r,PopNum)
      fmin = min(data,[],1);
    fmax = max(data,[],1);
